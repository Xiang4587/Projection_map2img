...
static float fx,
		...
		cx,
		cy;
// Add intrisic parameters
static float k1,k2,k3,t1,t2; //k:radial distortion coefficients , t:tangential distortion coefficients
...
#define SignalLampRadius 0.3  //0.3 revised
...
void cameraInfoCallback(const sensor_msgs::CameraInfo::ConstPtr camInfoMsg)
{
	...
	// Add k:radial distortion coefficients , t:tangential distortion coefficients
	k1 = static_cast<float>(camInfoMsg->D[0]);
	k2 = static_cast<float>(camInfoMsg->D[1]);
	k3 = static_cast<float>(camInfoMsg->D[4]);
	t1 = static_cast<float>(camInfoMsg->D[2]);
	t2 = static_cast<float>(camInfoMsg->D[3]);
}
...
void getTransform(Eigen::Quaternionf &ori, Point3 &pos)
{
	...
	listener.waitForTransform("camera_f", "map", now, ros::Duration(10.0));
	listener.lookupTransform("camera_f", "map", now, trf);

	tf::Vector3 &p = trf.getOrigin();    // Get the translation from map to camera
	tf::Quaternion o = trf.getRotation(); // Get the rotation from map to camera
	pos.x() = p.x();
	pos.y() = p.y();
	pos.z() = p.z();
	ori.w() = o.w();
...
bool project2(const Point3 &pt, int &u, int &v, bool useOpenGLCoord = false)
{
	...
	float _Xc = _pt.x() / _pt.z();
	float _Yc = _pt.y() / _pt.z();
	float r = sqrt(pow(_Xc,2) + pow(_Yc,2));

    float x_dis = _Xc*(1 + (k1*pow(r,2) + k2*pow(r,4) + k3*pow(r,6))) + 2*t1*_Xc*_Yc + t2*(pow(r,2) + 2*pow(_Xc,2));
	float y_dis = _Yc*(1 + (k1*pow(r,2) + k2*pow(r,4) + k3*pow(r,6))) + 2*t2*_Xc*_Yc + t1*(pow(r,2) + 2*pow(_Yc,2));

    float _u = fx*x_dis + cx;
	float _v = fy*y_dis + cy;
	...
	for (const auto& signal_map : vmap.signals)
	{
		...
		if (project2(signalcenter, u, v, useOpenGLCoord) == true)
		{
			...
			*/
            ...
            //ROS_INFO("Signal_angle: %.2f", signal_angle);
			...
			//std::cout << "Signal Angle: " << signalsInFrame.Signals << 
		}
	}
